...
function expandCliFlags(flags) {
  const {
    ...
    version
        ...
        installOptions = _objectWithoutProperties(flags, ["source", "help", "version"]);
...
async function writeLockfile(loc, importMap) {
  fs.writeFileSync(loc, JSON.stringify(importMap, undefined, 2), {
    encoding: 'utf8'
  });
}
...
async function resolveTargetsFromRemoteCDN(installTargets, lockfile, pkgManifest, config) {
  ...
  for (const installSpecifier of allInstallSpecifiers) {
    const installSemver = (pkgManifest.dependencies || {})[installSpecifier] || (pkgManifest.devDependencies || {})[installSpecifier] || (pkgManifest.peerDependencies || {})[installSpecifier] || 'latest';
...
async function scanImports({
  ...
  return includeFiles.filter(filePath => filePath.endsWith('.js') || filePath.endsWith('mjs') || filePath.endsWith('.ts') || filePath.endsWith('.tsx')).map(filePath => [filePath, fs.readFileSync(filePath, 'utf8')]).map(([filePath, code]) => getInstallTargetsForFile(filePath, code)).reduce((flat, item) => flat.concat(item), []).sort((impA, impB) => impA.specifier.localeCompare(impB.specifier));
}
...
async function cli(args) {
  ...
  if (source === 'pika') {
    newLockfile = await resolveTargetsFromRemoteCDN(installTargets, lockfile, pkgManifest);
  }
